...
pub async fn get_branding<S: AdminState>(State(state): State<S>) -> Response {
...
pub async fn get_public_branding<S: AdminState>(State(state): State<S>) -> Response {
    let branding = load_branding(&state);
    (StatusCode::OK, axum::Json(branding)).into_response()
}
...
pub async fn update_branding<S: AdminState>(
    State(state): State<S>,
...
pub async fn reset_branding<S: AdminState>(State(state): State<S>) -> Response {
...
pub fn load_branding<S: AdminState>(state: &S) -> BrandingConfig {
    state.branding_store().load()
}

/// Save branding config to the preferences table.
fn save_branding<S: AdminState>(state: &S, config: &BrandingConfig) {
