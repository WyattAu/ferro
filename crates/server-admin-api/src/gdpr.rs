...
pub async fn list_gdpr_requests<S: AdminState>(State(state): State<S>) -> Response {
    let store = match state.db() {
        Some(db) => GdprStore::new().with_db(db.clone()),
        None => GdprStore::new(),
    };
...
async fn process_data_export<S: AdminState>(state: &S, request_id: &str, user_id: &str) {
    let store = match state.db() {
        Some(db) => GdprStore::new().with_db(db.clone()),
        None => return,
    };
...
async fn process_data_erasure<S: AdminState>(state: &S, request_id: &str, user_id: &str) {
    let store = match state.db() {
        Some(db) => GdprStore::new().with_db(db.clone()),
        None => return,
    };
...
async fn list_user_files<S: common::server_context::HasStorage>(
    state: &S,
    _user_id: &str,
