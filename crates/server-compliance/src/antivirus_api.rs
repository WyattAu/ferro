...
pub async fn antivirus_status<S: ComplianceState>(State(_state): State<S>) -> Response {
    let config = ClamavTcpConfig::default();
...
pub async fn scan_history<S: ComplianceState>(State(_state): State<S>) -> Response {
    let history = ScanHistory::new();
