    ...
    let claims = match (&oidc, token) {
        (Some(validator), Some(t)) => match validator.validate_token(t).await {
            ...
            Err(e) => {
                ...
                return unauthorized_response("TOKEN_INVALID", "Token validation failed");
            }
        },
        (Some(_), None) => {
            return unauthorized_response("AUTH_REQUIRED", "Missing Bearer token");
        }
...
mod tests {
    ...
    fn test_decode_claims_unsafe_expired() {
        let claims = make_claims("alice", 1000);
        let token = encode_claims_unsafe(&claims);
