...
// Re-export base64 for the auth guard's credential parsing.
use base64::Engine;
...
/// Tracks failed authentication attempts per IP + username pair.
pub struct AuthAttemptTracker {
    /// Map from "ip:username" -> (fail_count, first_fail_time, locked_until)
...
impl AuthAttemptTracker {
    /// Record a failed authentication attempt. Returns true if the account
    /// is now locked.
...
// ---------------------------------------------------------------------------
// Login-specific rate limiter (stricter than global rate limiter)
// ---------------------------------------------------------------------------

/// Separate rate limiter for authentication endpoints.
/// 5 attempts per minute per IP (vs 10,000/min globally).
...
/// Reserved filenames that are disallowed (Windows compatibility).
...
fn is_invalid_char(c: char) -> bool {
    // Control characters (C0 and C1, except TAB)
    (c.is_control() && c != '\t') || c == '\x00'
}

/// Validate a file/directory name for safety.
/// Returns `Ok(())` if the name is valid, or an error description.
pub fn validate_filename(name: &str) -> Result<(), &'static str> {
    ...
    // Check for reserved names (case-sensitive: Linux server).
    // Windows compatibility only blocks exact uppercase matches.
    ...
    // Check for path separators (defense in depth)
...
/// Validate a full path, checking each component.
/// Returns `Ok(())` if all components are valid.
pub fn validate_path(path: &str) -> Result<(), &'static str> {
    for component in path.split('/') {
        if component.is_empty() {
            continue; // Leading/trailing slashes are fine
        }
...
/// A small set of high-risk content types that should be verified.
/// This is not exhaustive but covers the most common attack vectors.
static MAGIC_SIGNATURES: &[MagicSignature] = &[
    // ZIP (covers .docx, .xlsx, .pptx, .jar, .apk, .odt, etc.)
    MagicSignature {
        offset: 0,
        bytes: b"PK\x03\x04",
    ...
    // Script shebangs (text files with shebang)
    MagicSignature {
        offset: 0,
        bytes: b"#!",
...
/// Verify that the actual file content matches the declared Content-Type.
/// Returns `Some(detected_type)` if the types don't match, or `None` if they
/// match or the type cannot be verified.
pub fn verify_content_type(declared: &str, data: &[u8]) -> Option<String> {
    // Skip verification for types we can't check
    ...
    for sig in MAGIC_SIGNATURES {
        ...
        {
            // Match found -- check if it's compatible with declared type
    ...
    None // No magic signature matched, can't verify
}

/// Check if two content types are compatible (e.g., "image/jpeg" and "image/png"
/// are both images, so we allow either).
fn types_compatible(declared: &str, detected: &str) -> bool {
    ...
    // application/zip covers many formats (docx, xlsx, etc.)
...
/// Infer content type from magic bytes. Returns `None` if unrecognized.
...
/// Generate a CSRF token (cryptographically random 32-byte hex string).
...
/// Extract the client IP from the request headers (X-Forwarded-For or
/// fallback to "unknown").
...
/// Extract the username from a Basic Authorization header, if present.
...
/// Middleware that enforces account lockout and login rate limiting.
///
/// This must be layered **outside** (before) the `simple_auth_middleware` in
/// the middleware stack so that locked-out clients receive a 429/403 response
/// before credentials are checked.
///
/// Uses `from_fn_with_state::<crate::AppState, _, _>` so that the
/// `AuthAttemptTracker` and `LoginRateLimiter` are available via state.
pub async fn auth_guard_middleware(
    ...
    // Skip guard for public paths -- they do not carry credentials.
    ...
    // Check lockout BEFORE attempting auth (rejects locked accounts early)
    if state
    ...
    // Run the next middleware (simple_auth) and inspect the response.
    let response = next.run(req).await;
    let status = response.status();

    if status == axum::http::StatusCode::UNAUTHORIZED {
        // Check rate limiter on failure: reject if too many recent failures
...
mod tests {
    ...
    fn test_validate_filename_reserved_names() {
        ...
        assert!(validate_filename("con").is_ok()); // lowercase is ok for non-Windows
    ...
    fn test_auth_tracker_lockout() {
        ...
        assert!(!tracker.record_failure("1.2.3.4", "admin"));
        assert!(tracker.record_failure("1.2.3.4", "admin")); // 3rd = locked
        assert!(tracker.is_locked_out("1.2.3.4", "admin"));
    ...
    fn test_auth_tracker_different_ips() {
        ...
        assert!(!tracker.record_failure("2.2.2.2", "admin")); // Different IP
        // Neither should be locked (each IP has 1 failure)
    ...
    fn test_login_rate_limiter() {
        ...
        assert!(rt.block_on(limiter.check("1.2.3.4"))); // 2 remaining
        assert!(rt.block_on(limiter.check("1.2.3.4"))); // 1 remaining
        assert!(rt.block_on(limiter.check("1.2.3.4"))); // 0 remaining
        assert!(!rt.block_on(limiter.check("1.2.3.4"))); // blocked
    ...
    fn test_detect_content_type_mp4() {
        // Minimal MP4: 4 bytes padding + "ftyp"
        let data = [0u8; 8];
        let mut data = data;
