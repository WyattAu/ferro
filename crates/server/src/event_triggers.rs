...
impl WasmEventTriggerStore {
    ...
    pub fn with_db(mut self, db: crate::db::DbHandle) -> Self {
        self.db = Some(db);
        self
    }
}
...
pub async fn delete_event_trigger(Path(id): Path<String>) -> Response {
    if trigger_store().remove(&id).await {
        ...
        ApiError::not_found(ApiError::NOT_FOUND, "Trigger not found")
    }
}
...
pub async fn fire_event_triggers(state: &AppState, event_type: EventType, path: &str, owner: &str) {
...
mod tests {
    ...
    fn test_event_type_as_str() {
        ...
        assert_eq!(EventType::FileModified.as_str(), "file.modified");
        assert_eq!(EventType::ShareCreated.as_str(), "share.created");
        assert_eq!(EventType::FileLocked.as_str(), "file.locked");
    }
    ...
    #[tokio::test]
    async fn test_store_find_matching() {
        let store = WasmEventTriggerStore::new();
        store
            .add(EventTrigger {
                id: "t1".to_string(),
                event_type: EventType::FileUploaded,
                worker_name: "w.wasm".to_string(),
                path_pattern: "*.pdf".to_string(),
                enabled: true,
                created_at: "2026-01-01T00:00:00Z".to_string(),
                success_count: 0,
    ...
    async fn test_store_toggle() {
        ...
        assert!(!store.toggle("nonexistent").await);
    }

    #[tokio::test]
    async fn test_store_counters() {
        let store = WasmEventTriggerStore::new();
        store
            .add(EventTrigger {
                id: "t1".to_string(),
        ...
        store.record_success("t1").await;
        store.record_success("t1").await;
        store.record_failure("t1").await;
