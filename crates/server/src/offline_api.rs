...
pub async fn get_status(State(state): State<AppState>) -> Response {
    let pending_count = match state.offline_queue {
        Some(ref q) => q.pending_count().await,
        None => 0,
    };
...
pub async fn list_cached(State(state): State<AppState>) -> Response {
    let cache = state.offline_cache.read().await;
...
mod tests {
    ...
    #[tokio::test]
    async fn test_get_status_no_queue() {
    ...
    #[tokio::test]
    async fn test_list_pending_no_queue() {
        let state = AppState::in_memory();
        let response = list_pending(State(state)).await;
        assert_eq!(response.status(), StatusCode::OK);
    }

    #[tokio::test]
    async fn test_list_cached_empty() {
        let state = AppState::in_memory();
        let response = list_cached(State(state)).await;
        assert_eq!(response.status(), StatusCode::OK);
    }

    #[tokio::test]
    async fn test_trigger_sync_no_queue() {
        let state = AppState::in_memory();
