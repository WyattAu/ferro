...
use crate::{AuditEntry, AutomationState};
...
pub async fn create_policy(
    ...
    if req.name.trim().is_empty() {
        return error_bad_request("Policy name must not be empty");
    }
    ...
    if let Some(ref db) = state.db {
        ...
        if let Err(e) = conn.execute(
            ...
            return error_internal("Failed to create retention policy");
        }
    } else {
...
pub async fn delete_policy(
    ...
    if let Some(ref db) = state.db {
        ...
        match affected {
            Ok(0) => return error_not_found("Policy not found"),
            Ok(_) => return (StatusCode::NO_CONTENT, "").into_response(),
            Err(e) => {
                warn!(error = %e, "failed to delete retention policy");
                return error_internal("Failed to delete retention policy");
            }
        }
    }
...
pub async fn execute_policies(Extension(state): Extension<Arc<AutomationState>>) -> Response {
...
fn update_last_run(state: &AutomationState) {
    if let Some(ref db) = state.db {
        let conn = db.lock().unwrap_or_else(|e| e.into_inner());
...
pub fn spawn_retention_daemon(
    ...
    tokio::spawn(async move {
        ...
        if !cancel.is_cancelled() {
            run_retention_check(&state).await;
        }
...
async fn run_retention_check(state: &AutomationState) {
    let policies = load_enabled_policies(state);
    if policies.is_empty() {
        return;
    }
