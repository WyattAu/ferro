//! OCR text extraction for PDF files using the `pdf` crate.
//!
//! Extracts text content from PDF files for search indexing.
//! Falls back to empty string for non-PDF or unreadable files.

use tracing::debug;

/// Extract text content from a PDF or image for search indexing.
///
/// For PDFs, parses the content stream and extracts text from `Tj` and `TJ` operators.
/// For images and unsupported types, returns an empty string (OCR requires external tools).
pub fn extract_text(content: &[u8], content_type: &str) -> String {
    match content_type {
        "application/pdf" => extract_pdf_text(content),
        ct if ct.starts_with("image/") => {
            // Image OCR requires tesseract or similar — not available.
...
/// Extract text from a PDF by parsing content stream operators.
///
/// Iterates through all pages and collects text from `Tj` (TextDraw)
/// and `TJ` (TextDrawAdjusted) operators. Text is joined with spaces/newlines.
