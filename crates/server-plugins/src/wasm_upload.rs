...
pub async fn upload_wasm_module<S: PluginState>(
    ...
    while let Ok(Some(field)) = multipart.next_field().await {
        ...
        match ferro_core::fs_util::atomic_write_async(dest_path.clone(), data.to_vec()).await {
            Ok(_) => {
...
pub async fn list_wasm_modules<S: PluginState>(State(state): State<S>) -> Response {
    let workers_dir = match state.workers_dir() {
        Some(dir) => dir.clone(),
        None => {
            return (
...
mod tests {
    ...
    // Minimal test state for unit tests
    #[derive(Clone)]
    struct TestState {
    ...
    impl PluginState for TestState {
        ...
        fn wasm_runtime(&self) -> Option<&std::sync::Arc<ferro_core::wasm::WasmWorkerRuntime>> {
            None
        }
    }
    ...
    async fn test_upload_list_delete_flow() {
        ...
        let app = axum::Router::new()
            .route(
                "/api/workers/upload",
                axum::routing::post(upload_wasm_module::<TestState>),
            )
    ...
    async fn test_upload_no_workers_dir_returns_503() {
        let state = TestState { workers_dir: None };
