...
pub async fn create_snapshot<S: StorageUtilsState>(
    State(state): State<S>,
...
pub async fn list_snapshots<S: StorageUtilsState>(State(state): State<S>) -> Response {
...
pub async fn restore_snapshot<S: StorageUtilsState>(
    State(state): State<S>,
    axum::extract::Path(id): axum::extract::Path<String>,
) -> Response {
