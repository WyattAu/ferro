        ...
        spawn_local(async move {
            if let Some(window) = web_sys::window() {
                let cb = wasm_bindgen::closure::Closure::wrap(Box::new(
                    move |msg: String, _source: String, _lineno: u32, _colno: u32, _error: wasm_bindgen::JsValue| {
                ...
                let cb2 = wasm_bindgen::closure::Closure::wrap(Box::new(move |ev: wasm_bindgen::JsValue| {
                ...
                let _ = window.add_event_listener_with_callback("unhandledrejection", cb2.as_ref().unchecked_ref());
                cb2.forget();
            }
        });
    }
        ...
        {move || error_msg.get().map(|msg| view! {
            <div class="fixed inset-0 z-[9999] bg-[var(--bg-base)] flex items-center justify-center p-4" role="alertdialog" aria-modal="true" aria-labelledby="error-boundary-title" aria-describedby="error-boundary-desc">
                <div class="max-w-md w-full text-center">
                    ...
                    <div class="bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-lg p-4 mb-6 text-left">
