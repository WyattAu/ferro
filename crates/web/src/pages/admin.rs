            ...
            <div class="max-w-7xl mx-auto py-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">"Storage"</h2>
                        <StorageStatsCard />
                    </div>

                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">"Share Links"</h2>
                        <ShareLinksCard />
                    </div>
    ...
    view! {
        <div>
            {move || loading.get().then(|| view! {
                <div class="text-sm text-gray-500 dark:text-gray-400" role="status" aria-live="polite">"Loading..."</div>
            })}
            {move || stats.get().map(|s| view! {
                <div class="space-y-3">
                    <div class="flex justify-between">
                        <span class="text-gray-600 dark:text-gray-400">"Files"</span>
                        <span class="font-medium text-gray-900 dark:text-gray-100">{s.get("files").and_then(|v| v.as_u64()).unwrap_or(0)}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600 dark:text-gray-400">"Collections"</span>
                        <span class="font-medium text-gray-900 dark:text-gray-100">{s.get("collections").and_then(|v| v.as_u64()).unwrap_or(0)}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600 dark:text-gray-400">"Total Size"</span>
                        <span class="font-medium text-gray-900 dark:text-gray-100">{format_size(s.get("total_bytes").and_then(|v| v.as_u64()).unwrap_or(0))}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600 dark:text-gray-400">"CAS Dedup"</span>
                        <span class={if s.get("cas").and_then(|c| c.get("enabled")).and_then(|e| e.as_bool()).unwrap_or(false) { "text-green-600 dark:text-green-400" } else { "text-gray-500 dark:text-gray-400" }}>
    ...
    view! {
        <div>
            ...
            {move || (!loading.get() && shares.with(Vec::is_empty)).then(|| view! {
                <div class="text-sm text-gray-500 dark:text-gray-400">"No active share links"</div>
            })}
            <For
                ...
                {move || {
                    ...
                    view! {
                        <div class="py-2 border-b border-gray-100 dark:border-gray-700 last:border-0">
                            <div class="text-sm font-medium text-gray-900 dark:text-gray-100">{path}</div>
                            <div class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">"Expires: " {expires}</div>
                        </div>
                    }
                }}
            </For>
            ...
            >
                {move || {
                    ...
                    let color = match status {
                        ...
                        400..=499 => "text-yellow-600 dark:text-yellow-400",
                    ...
                    view! {
                        <div class="py-1.5 border-b border-gray-100 dark:border-gray-700 last:border-0 text-xs">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-gray-600 dark:text-gray-400 truncate">{path}</span>
                                <span class="text-gray-500 dark:text-gray-500 ml-auto">{user}</span>
                            </div>
                        </div>
                    }
                }}
