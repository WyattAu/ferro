...
) -> Result<(axum::http::StatusCode, Json<ScimUser>), ScimError> {
...
pub async fn list_groups(
    ...
    Ok(Json(ScimListResponse {
        schemas: vec!["urn:ietf:params:scim:api:messages:2.0:ListResponse".into()],
        total_results: state.group_store.count(),
        start_index: 1,
        items_per_page: groups.len() as u32,
        resources: groups,
    }))
}
...
) -> Result<(axum::http::StatusCode, Json<ScimGroup>), ScimError> {
