...
pub fn load_file_config(config_path: Option<&str>) -> anyhow::Result<FileConfigValues> {
    if let Some(path) = config_path {
        ...
        Ok(FileConfigValues::default())
    }
}

pub fn validate_config(cli: &ServerConfig, file_config: &FileConfigValues) -> anyhow::Result<()> {
